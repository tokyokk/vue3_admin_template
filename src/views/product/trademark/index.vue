    ...
    <el-card class="box-card">
        ...
        <el-table border stripe style="margin: 10px 0" :data="trademarkArr">
            ...
            <el-table-column label="品牌名称">
                <template #="{ row, $index }">
                    <pre style="color: darksalmon">{{ row.tmName }}</pre>
                </template>
            </el-table-column>
        ...
        pagination
			...
			:small:是否使用小型分页器,默认不使用,true使用,false不使用!
			:background:设置分页器按钮的背景颜色,true添加,false不添加
			layout:可以设置分页器六个子组件的布局调整
            :page-count="9":可以设置固定数字模拟数据
        -->
        <el-pagination
    ...
    <el-dialog v-model="dialogFormVisible" :title="trademarkParams.id ? '修改品牌' : '添加品牌'">
        <el-form style="width: 80%" :model="trademarkParams" :rules="rules" ref="formRef">
            ...
            <el-form-item label="品牌LOGO" label-width="100px" prop="logoUrl">
                <!--
				upload组件:
					action:请求URL,图片伤残路径书写/api,否则代理服务器不发送这次post请求
					show-file-list:是否显示已上传文件列表
					:before-upload:限制用户上传文件的格式和大小
				-->
                <el-upload
...
const confirm = async () => {
    // 在你发请求之前,要对整个表单进行校验
    // 调用这个方法进行全部表单的校验,如果全部校验通过,在执行后面的语法
...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
    ...
    if (
        ...
        ElMessage({
            type: 'error',
            message: '上传文件的格式务必是PNG|JPEG|GIF'
        })
        return false
    }
}
...
const rules = {
    /**
     * required:必填字段的校验,表单前面会出现五角星
     * trigger:代表触发校验规则实际[blur/change]
     */
